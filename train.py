    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if best_acc < max(self.val_acc['epoch']):
            ...
            predict_test = np.argmax(model.predict(
                [X_test_psd, X_test_eeg]), axis=1)
            cm = confusion_matrix(y_test_eeg, predict_test,
                                  labels=[0, 1, 2, 3, 4])
            print(cm)
            over_cm[-1] = cm
...
def train(args):
    global fold_string, best_acc, model, file_dir, over_cm, X_test_eeg, y_test_eeg, X_test_psd, y_test_psd
    ...
    for fold_idx in range(real_fold):
        ...
        # Reading Data
        data_loader_eeg = SeqDataLoader(
            data_dir, num_folds, fold_idx, classes, n_files)
        X_train_eeg, y_train_eeg, X_test_eeg, y_test_eeg = data_loader_eeg.load_data()

        X_train_eeg = X_train_eeg[:, :6, :]
        X_test_eeg = X_test_eeg[:, :6, :]
        ...
        model = BrainSleepNet.create_BrainSleepNet(
        ...
        model.fit([X_train_psd, X_train_eeg], y_train_eeg, validation_data=([X_test_psd, X_test_eeg], y_test_eeg), epochs=args.epoch, batch_size=args.batch_size, callbacks=callbacks_list, verbose=2, shuffle=True)

        del X_train_eeg, y_train_eeg, X_test_eeg, y_test_eeg, model, data_loader_eeg, data_loader_eeg, X_train_psd, y_train_psd, X_test_psd, y_test_psd, data_loader_psd
...
def main():
    ...
    parser.add_argument('--data_dir1', type=str, default='./data/EEG', metavar='N',
                        help='data_dir1 (default: ./data/EEG)')
    parser.add_argument('--data_dir2', type=str, default='./data/fre_spa', metavar='N',
                        help='data_dir2 (default:./data/fre_spa)')
